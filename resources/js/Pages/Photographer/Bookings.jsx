...
import { CalendarCheck, ChevronRight, Camera, Search, User } from 'lucide-react';

export default function PhotographerBookings({ bookings, currentStatus = 'all' }) {
    const statuses = [
        { id: 'all', label: 'All Orders' },
        { id: 'pending', label: 'Pending Requests' },
        { id: 'accepted', label: 'Accepted (Awaiting Advance)' },
    ...
    const filterStatus = (status) => {
        if (status === 'all') {
            router.get('/photographer/bookings');
        } else {
            router.get('/photographer/bookings', { status: status });
        }
    };

    return (
        <PhotographerLayout title="Orders & Bookings">
            
            <div className="mb-6">
                <h1 className="text-2xl font-extrabold text-white">Client Bookings & Orders</h1>
                <p className="text-xs text-slate-400 mt-0.5">Accept requests, coordinate shoot dates, and update progress.</p>
            </div>
            ...
            {bookings.data && bookings.data.length > 0 ? (
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <h3 className="font-bold text-sm text-slate-200 mt-1">
                                        Client: {b.customer?.name} ({b.customer?.phone || b.customer?.email})
                                    </h3>
                ...
                <div className="text-center py-16 bg-slate-900/40 rounded-3xl border border-slate-800 p-8">
                    ...
                    <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
                        No client bookings in this status filter.
                    </p>
                </div>
            )}
